...
AMDI({ cmd: ["alive", "hi", "online"], desc: Lang.AliveDesc, type: "primary", react: "💃🏻" }, (async (amdiWA) => {
    ...
    if (ALIVE_MSG.input === 'default' || ALIVE_MSG.input == undefined) {
        const buttons = [
            {type: "url", displayText: "💃🏻 Official Website", url: 'https://github.com'},
            {type: "url", displayText: "🎞️ YouTube Channel", url: 'https://www.youtube.com/@dileepatech'},
...
AMDI({ cmd: "system", desc: "Bot Status", cmdHideInMenu: true }, (async (amdiWA) => {
    ...
    await reply('```⚕️DILEEPATECH 4.0.0v - Multi-Device - Status⚕️```\n\n' + Lang.upTime + uptime + `\n` + Lang.ping + (end - start) +'\n'+ Lang.memUsage + usage, "💻");
    return await amdiWA.web.sendMessage(amdiWA.clientJID, { delete: checkSTS.key });
}));
...
AMDI({ cmd: ["qaversion", "version"], desc: "Version check", cmdHideInMenu: true }, (async (amdiWA) => {
    ...
    return await reply(`*🧬DILEEPATECH 4.0.0v - Multi-Device Version 🧬*\n\n` + '```Installed version``` : ' + version +'\n' + '\n```Official Site``` : https://www.youtube.com/@dileepatech');
}));